...
// 合併資料 (確保資料檔案正常 export 且路徑正確)
const allEncyclopediaData = [
  ...(oilData || []), 
  ...(acuData || []), 
  ...(herbData || []), 
  ...(formulaData || [])
  ...
  const renderFormattedText = (text) => {
    ...
    const parseBoldSyntax = (str) => {
      ...
      return parts.map((part, i) => {
        if (part.startsWith('==') && part.endsWith('==')) return <mark key={i} className="bg-[#F3E1C5] text-[#2C3C30] px-1 py-0.5 rounded-md font-bold mx-0.5">{part.slice(2, -2)}</mark>;
      ...
      if (listMatch) {
        return (
          <div key={index} className="flex items-start gap-2 mb-1 last:mb-0 text-justify text-sm text-[#6B7A6E]">
            <span className="shrink-0 font-bold w-6">{listMatch[1]}</span>
            <div className="flex-1 break-words">{parseBoldSyntax(trimmed.substring(listMatch[0].length))}</div>
          </div>
        );
      }
      return <p key={index} className="text-justify break-words mb-1 last:mb-0 text-sm text-[#6B7A6E]">{parseBoldSyntax(trimmed)}</p>;
    });
  };
    ...
    {cat}
      ...
      <main className="max-w-5xl mx-auto">
        {filteredData.length > 0 ? (
              ...
              >
                <div>
                  <div className="flex flex-wrap gap-1.5 items-start mb-3">
                    {item.category === "精油" ? (
                      <>
                        <span className="text-xs font-medium tracking-wider px-2.5 py-1 rounded bg-[#EAE7E0] text-[#6B7A6E]">{item.constitutionTag}</span>
                        <span className="text-xs font-medium tracking-wider px-2.5 py-1 rounded bg-[#E5EAE6] text-[#4E6654]">{item.chemicalTag}</span>
                      </>
                    ) : (
                      <span className="text-xs font-medium tracking-wider px-2.5 py-1 rounded bg-[#F0EDE6] text-[#3A4F3F]">{item.tag}</span>
                    )}
                  </div>
                  <h3 className="text-2xl font-bold text-[#3A4F3F] group-hover:text-[#A39284] transition-colors duration-200">{item.name}</h3>
                  <p className="text-sm italic text-[#A39284] mt-1 mb-4 font-serif">{item.category === "精油" ? item.englishName : (item.acuTable?.code || '')}</p>
                  <div className="mb-4">{renderFormattedText(item.description)}</div>
                </div>
          ...
          <div className="text-center py-20 text-[#A39284] bg-white rounded-2xl border border-dashed border-[#E5E0D8]">沒有找到符合條件的資料。</div>
        )}
      </main>
