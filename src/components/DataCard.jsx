...
const DataCard = memo(function DataCard({ item = {}, onSelectItem }) {
  ...
  const rawLocation = String(firstValue(item, ['acuDetails.location', 'acuTable.location', 'location']) || '').trim();
  const locationLabel = isAcupuncture ? getLocationLabel(rawLocation) : '';
  const tags = useMemo(() => {
  ...
  return <div onClick={handleClick} role="button" tabIndex={0} onKeyDown={handleKeyDown} className="group relative cursor-pointer overflow-hidden rounded-[1.75rem] border border-white/70 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md md:p-7">
    ...
    {isOil && <div className="mt-3 space-y-3"><div className="break-words text-[15px] italic text-[#6B7280]">{englishName || ''}</div><div className="line-clamp-3 whitespace-pre-wrap break-words text-[15px] leading-6 text-[#3A4F3F]">{indications || ''}</div></div>}
