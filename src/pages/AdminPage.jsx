...
import AddEntryPage from '../features/entries/pages/AddEntryPage.jsx';
import OilModal from '../components/OilModal.jsx';
import AcuModal from '../components/AcuModal.jsx';
import HerbModal from '../components/HerbModal.jsx';
import FormulaModal from '../components/FormulaModal.jsx';
import BookModal from '../components/BookModal.jsx';
import OtherDetailPage from '../components/OtherDetailPage.jsx';

import { APP_VERSION } from '../generatedVersion.js';

const CardViewer = lazy(
  () => import('../components/CardViewer.jsx')
);
...
const getItemIdentity = (item) => {
  ...
  if (category && name) {
    return `category:${category}__name:${name}`;
  }
  ...
  {normalizeCategory(
    ...
    : getCategoryLabel(
      ...
      <div className="flex flex-wrap justify-end gap-2 print:hidden">
        <button type="button" onClick={() => onViewItem(item)}disabled={disabled}
        ...
        <button type="button"onClick={() => onViewCard(item)}disabled={disabled}
          className="rounded-lg bg-[#F7F5F0] px-4 py-2 text-sm font-medium text-[#3A4F3F] hover:bg-[#E5E0D8] disabled:cursor-not-allowed disabled:opacity-50"
        >
          圖卡
        </button>

        <button type="button"onClick={() => onEdit(item)}disabled={disabled}
        ...
        <button type="button"onClick={() => onDelete(item)}disabled={disabled}
...
export default function AdminPage({
  ...
  const addItem = (
    ...
    const mergedItem = {
      ...
      firestoreId:
        normalizedItem.firestoreId ||
        previousItem.firestoreId ||
        '',

      entryKey:
        normalizedItem.entryKey ||
        previousItem.entryKey ||
        '',
      ...
      oilDetails: {
        ...(previousItem.oilDetails || {}),
        ...(normalizedItem.oilDetails || {}),
      },

      acuTable: {
        ...(previousItem.acuTable || {}),
        ...(normalizedItem.acuTable || {}),
      },

      acuDetails: {
        ...(previousItem.acuDetails || {}),
        ...(normalizedItem.acuDetails || {}),
      },

      bookDetails: {
        ...(previousItem.bookDetails || {}),
        ...(normalizedItem.bookDetails || {}),
      },

      knowledgeDetails: {
        ...(previousItem.knowledgeDetails || {}),
        ...(normalizedItem.knowledgeDetails || {}),
      },
    };
